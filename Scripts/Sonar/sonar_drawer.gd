...
# organic sonar contact styling
var organic_lifetime := .7
var organic_outer_radius := 5.0
var organic_core_radius := 2.3
var organic_color_outer := Color(0.58, 0.86, 0.22, 0.24)
var organic_color_mid := Color(0.74, 0.95, 0.28, 0.42)
var organic_color_core := Color(0.88, 1.0, 0.45, 0.85)
var organic_jitter := 2.8
	...
	for pulse in pulses:
		...
		for echo in pulse.echoes:
			...
			if pulse.time >= echo.delay:
				...
				if echo.get("organic", false):
					register_organic_hit(p)
				else:
					register_hit(p)

	pulses = pulses.filter(func(p): return p.time < pulse_lifetime)
	organic_echoes = organic_echoes.filter(func(entry):
		entry["age"] += delta
		return entry["age"] < organic_lifetime
	)
...
func register_organic_hit(world_point: Vector2):
	organic_echoes.append({
		"point": world_point + Vector2(
			randf_range(-organic_jitter, organic_jitter),
			randf_range(-organic_jitter, organic_jitter)
		),
		"age": 0.0,
		"seed": randf() * 2000.0
...
func draw_organic_echoes():
	for entry in organic_echoes:
		var t = 1.0 - (entry["age"] / organic_lifetime)
		if t <= 0.0:
			continue

		var world_point: Vector2 = entry["point"]
		var seedNoise: float = entry["seed"]

		var wobble_x = pseudo_noise(world_point + Vector2(seedNoise, simulation_time * 17.0))
		var wobble_y = pseudo_noise(world_point + Vector2(simulation_time * 23.0, seedNoise))
		var center = to_local(world_point + Vector2(wobble_x, wobble_y) * 1.6)

		var outer_color = organic_color_outer
		outer_color.a *= t * t
		var mid_color = organic_color_mid
		mid_color.a *= t
		var core_color = organic_color_core
		core_color.a *= t

		var outer_radius = organic_outer_radius * (0.7 + t * 0.8)
		var core_radius = organic_core_radius * (0.8 + t * 0.4)

		draw_circle(center, outer_radius, outer_color)
		draw_circle(center + Vector2(1.5, -1.0), outer_radius * 0.62, mid_color)
