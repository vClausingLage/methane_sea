...
@export var fill_height := 300.0:
	set(value):
		fill_height = maxf(value, 1.0)
...
func generate() -> void:
	...
	if path == null or polygon == null:
		push_warning("CaveCeiling.gd: Missing required nodes Path2D and Polygon2D.")
		return
	...
	var top_y := _get_top_y(edge)
	for p in edge:
		fill_polygon.append(Vector2(p.x, top_y))

	for i in range(edge.size() - 1, -1, -1):
		fill_polygon.append(edge[i])
	...
	_apply_segment_collision(edge, top_y)
...
func _apply_segment_collision(edge: Array, top_y: float) -> void:
	var parent := _get_collision_parent()
	var part_index := 0
...
func _restore_collision_from_saved_edge() -> void:
	...
	_apply_segment_collision(edge, _get_saved_top_y(edge))


func _get_saved_top_y(edge: Array) -> float:
	if polygon != null and polygon.polygon.size() > 0:
		var top_y := polygon.polygon[0].y
		for point in polygon.polygon:
			top_y = minf(top_y, point.y)
		return top_y

	return _get_top_y(edge)
...
func _get_top_y(edge: Array) -> float:
	var top_y: float = edge[0].y
	for p in edge:
		top_y = minf(top_y, p.y)
	return top_y - fill_height
...
func _make_default_curve() -> Curve2D:
	...
	curve.add_point(Vector2(-900.0, -260.0))
	curve.add_point(Vector2(-450.0, -320.0))
	curve.add_point(Vector2(0.0, -240.0))
	curve.add_point(Vector2(450.0, -300.0))
	curve.add_point(Vector2(900.0, -220.0))
	return curve
